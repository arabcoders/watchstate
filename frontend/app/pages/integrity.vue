          ...
          <div class="field is-grouped">
            ...
            <p class="control">
              <button class="button is-danger" @click="massDelete" v-tooltip.bottom="'Delete selected records.'"
                :disabled="isDeleting || isLoading || selected_ids.length < 1" :class="{ 'is-loading': isDeleting }">
            ...
            <p class="control" v-if="isCached">
              <button class="button is-danger is-light" @click="emptyCache" v-tooltip.bottom="'Empty cache.'"
          ...
          <div class="card-content">
            <div class="content">
              <ul>
                ...
                <li>There are no path replacement support at the moment. The path must match what your media servers are
                  reporting. There are plans to add this feature in the future.
                </li>
                  ...
                  <div class="columns is-multiline is-mobile">
                    ...
                    <div class="column is-12" v-if="item?.integrity">
                      <div class="field is-grouped">
                        ...
                        <div class="control">
                          <NuxtLink v-for="record in item.integrity" :key="`${item.id}-int-${record.backend}`"
...
import { request, awaitElement, copyText, makeName, makeSearchLink, notification, TOOLTIP_DATE_FORMAT } from '~/utils'
...
const loadContent = async (): Promise<void> => {
  ...
  try {
    ...
    const json = await parse_api_response<{ items: Array<IntegrityItemWithUI>, fromCache?: boolean }>(response)
...
onMounted(() => {
  ...
  if (items.value.length < 1 && cache.has('integrity')) {
    const cachedData = cache.get('integrity') as { items: Array<IntegrityItemWithUI>, fromCache: boolean }
