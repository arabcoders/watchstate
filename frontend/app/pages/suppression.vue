  ...
  <div>
    <div class="columns is-multiline">
          ...
          <div class="card">
            <header class="card-header">
              <p class="card-header-title is-unselectable is-justify-center">
                <template v-if="formData.id"> Edit suppression rule </template>
                <template v-else> Add new suppression rule </template>
              </p>
            </header>
            ...
            <div class="card-content">
                ...
                <div class="control has-icons-left">
                  ...
                  <div class="icon is-small is-left">
                    <i
                      class="fa"
                      :class="{
                        'fa-code': 'regex' === formData.type,
                        'fa-heading': 'contains' === formData.type,
                      }"
                    ></i>
                  </div>
                  <p class="help">
                    <template v-if="'regex' === formData.type">
                      ...
                      <span>
                        <NuxtLink to="https://regex101.com/" target="_blank"
                          >this link</NuxtLink
                        > </span
                      ><span></span>. Select <code>PCRE2 (PHP >=7.3)</code> flavor.
                    </template>
              ...
              <div class="field">
                ...
                <div class="control has-icons-left">
                  ...
                  <p class="help">
                    The example text must trigger the supplied rule. This is used to test the rule.
      ...
      <div class="column is-12" v-if="false === isLoading && items.length < 1">
        ...
        >
          <p>
            No suppression rules were found. To add a new rule click the
            <span class="is-clickable icon"><i class="fa fa-add"></i></span> button on top right of
            this page.
          </p>
        </Message>
      </div>
      ...
      <div class="column is-12">
        ...
        >
          <ul>
            <li>
              The log suppressor work on almost everything that <strong>WatchState</strong> output.
              However, there are some exceptions.
            </li>
            <li>
              The use case for this feature, is that sometimes it's out of your hands to fix a
              problem, and the constant logging of the same error can be annoying. This feature
              allows you to suppress the error from being shown/recorded.
            </li>
            <li>
              ...
              message. The less rules you have, the better. Having many rules will lead to
  ...
  try {
    ...
    if (!response.ok) {
      notification(
        'error',
        'Error',
        `${json.error.code ?? response.status}: ${json.error.message ?? response.statusText}`,
      );
      return;
    }
