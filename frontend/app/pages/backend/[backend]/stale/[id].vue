    ...
    <div class="columns is-multiline">
      <div class="column is-12 is-clearfix is-unselectable">
        ...
        <div class="is-hidden-mobile">
          <span class="subtitle">
            This page will show items from local database that no longer exists in the remote
            backend library.
          </span>
        </div>
      </div>
      ...
      <div class="column is-12">
        <div class="columns is-multiline" v-if="0 < items.length">
          <div class="column is-12">
            ...
            >
              <b>WatchState</b>, has found '<span class="has-text-danger is-bold"
                ><u>{{ counts.stale }}</u></span
              >' items in local database, that no longer exists in <b>{{ remote.name }}</b> library
              <b>{{ remote.library?.title }}</b
              >.
            </Message>
          </div>
        ...
        <div class="column is-12" v-else>
          ...
          <template v-else>
            <Message
              ...
              icon="fas fa-check"
            >
              Great, WatchState checked '<u>{{ counts.local }}</u
              >' items against <b>{{ remote.name }}</b> library
              <b>{{ remote.library?.title }}</b> '<u>{{ counts.remote }}</u
              >' items and did not find any local reference that isn't in the remote library.
            </Message>
          </template>
        </div>
        ...
        <div class="column is-12">
          ...
          >
            <ul>
              <li>
                This page is used to show stale references to items in the local database compared
                to the backend.
              </li>
              <li>Remote data is cached in memory to speed up reloads.</li>
              <li>
                Is there harm in having stale references? there is no harm, however somethings might
                not work as expected if the item has changed id, for example, pushing via webhooks
                will fail as it's reference old item that no longer exists.
              </li>
            </ul>
          </Message>
        </div>
