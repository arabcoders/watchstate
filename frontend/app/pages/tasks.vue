...
const loadContent = async (): Promise<void> => {
  ...
  try {
    ...
    const json = await parse_api_response<{ tasks: Array<TaskItem>, queued: Array<string> }>(response)
    ...
    if ('error' in json) {
      notification('error', 'Error', `Tasks request error. ${json.error.code}: ${json.error.message}`)
      return
    }
