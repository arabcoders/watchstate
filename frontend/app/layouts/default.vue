      ...
      <div class="navbar-menu" :class="{ 'is-active': showMenu }">
        <div class="navbar-start">
          ...
          <NuxtLink
            ...
            @click="
              (e: Event) =>
                changeRoute(e, () => dEvent('history_main_link_clicked', { clear: true }))
            "
          >
            ...
            <div class="navbar-item" v-if="in_container">
              <button class="button is-dark" @click="showScheduler = !showScheduler">
                <span
                  class="icon"
                  :class="{
                    'has-text-primary': scheduler.status,
    ...
    <div>
      ...
      <div class="columns is-multiline is-mobile mt-3">
        ...
        <div class="column is-6 is-4-mobile has-text-right">
          <span
            v-tooltip="
              `Build Date: ${api_version_date}, Branch: ${api_version_branch}, commit: ${api_version_sha}`
            "
            v-if="api_version"
            class="has-tooltip"
          >
...
watch(
  () => api_version.value,
  () => (changelog_url.value = `/changelog?version=${api_version.value}`),
);
...
const applyPreferredColorScheme = (scheme: string): void => {
  ...
  for (let s = 0; s < document.styleSheets.length; s++) {
    ...
    for (let i = 0; i < cssRules.length; i++) {
      ...
      if (
        !mediaList ||
        typeof mediaList.appendMedium !== 'function' ||
        typeof mediaList.deleteMedium !== 'function'
      ) {
        continue;
      }
