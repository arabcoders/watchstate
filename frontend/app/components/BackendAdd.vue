<template>
  <Message title="Important" message_class="is-warning" icon="fas fa-info-circle" :newStyle="true">
    <ul>
      <li>
        If you are adding new backend that is fresh and doesn't have your current watch state, you
        should turn off import and enable only metadata import at the start to prevent overriding
        your current play state. Visit the following guide
    ...
    <div class="card">
      <div class="card-header">
        <p class="card-header-title">
          Add backend to '<u class="has-text-danger">{{ api_user }}</u
          >' user config.
        </p>
      </div>
      ...
      <div class="card-content">
        <div class="field" v-if="error">
          <Message
            title="Backend Error"
            id="backend_error"
            message_class="has-background-danger-80 has-text-dark"
        ...
        <template v-if="stage >= 0">
          <div class="field">
            ...
            <p class="help">
              The local user which this backend will be associated with. You can change this user
              via the
              <span class="icon"><i class="fas fa-users" /></span> users icon on top right of the
              page.
            </p>
          </div>
          ...
          <div class="field">
            ...
            <p class="help">
              ...
              Backend name must be in <strong>lower case a-z, 0-9 and _</strong> and cannot start
              with number.
            </p>
          </div>
              ...
              <div class="field">
                ...
                <p class="help">
                  <template v-if="'plex' === backend.type">
                    ...
                    <span class="is-bold"
                      >If you plan to add sub-users, YOU MUST use admin level token.</span
                    >
                  </template>
                  <template v-else>
                    Generate a new API Key from
                    <strong>Dashboard > Settings > API Keys</strong>.<br />
                    ...
                    should be used as last resort. and it's mostly untested. and things might not
                    work as expected.
                    <span class="is-bold has-text-danger"
                      >If you plan to add sub-users, YOU MUST use API KEY and not
                      ...
                      <span class="icon-text">
                        <span class="icon"
                          ><i
                            class="fas"
                            :class="{
                              'fa-check-double': !plex_oauth_loading,
                              'fa-spinner fa-pulse': plex_oauth_loading,
                            }"
                        /></span>
          ...
          <div class="field" v-if="'plex' === backend.type">
            ...
            <p class="help">
              If the user you are going to select has <strong>PIN</strong> enabled, you need to
              enter the pin here. Otherwise it will fail to authenticate.
            </p>
          </div>
        </template>
        ...
        <template v-if="stage >= 1">
          ...
          <template v-if="'plex' === backend.type">
            <div class="field">
              ...
              <div class="field-body">
                <div class="field">
                      ...
                      >
                        <span class="icon"
                          ><i
                            class="fa"
                            :class="{
                              'fa-spinner fa-spin': serversLoading,
                              'fa-refresh': !serversLoading,
                            }"
                        /></span>
                  ...
                  <p class="help">
                    Try to use non <strong>.plex.direct</strong> urls if possible, as they are often
                    have problems working in docker. If you use custom domain for your plex server
                    and it's not showing in the list, you can add it via Plex settings page.
                    <strong>Plex > Settings > Network > Custom server access URLs:</strong>. For
                    more information
                    <NuxtLink
                      target="_blank"
            ...
            <div class="field">
              <label class="label" for="backend_ownership"
                >Are you invited guest to this backend?</label
              >
              <div class="control">
                <input
                  ...
                  >
                    <span class="icon"
                      ><i
                        class="fa"
                        :class="{
                          'fa-spinner fa-spin': usersLoading,
                          'fa-refresh': !usersLoading,
                        }"
                    /></span>
                    <span class="is-hidden-mobile">Reload</span>
                  </button>
        ...
        <template v-if="stage >= 4">
          <div class="field" v-if="backend.import">
            <label class="label" for="backend_import"
              >Import play and progress updates from this backend?</label
            >
            <div class="control">
              <input
                id="backend_import"
          ...
          <div class="field" v-if="backend.import && !backend.import.enabled">
            <label class="label" for="backend_import_metadata"
              >Import metadata from this backend?</label
            >
            <div class="control">
              <input
            ...
            <p class="help has-text-danger is-bold">
              <span class="icon"><i class="fas fa-info-circle" /></span>
              As you have disabled the state import, you should enable this option for efficient and
              fast updates to this backend.
            </p>
          </div>

          <div class="field" v-if="backend.export">
            <label class="label" for="backend_export"
              >Send play and progress updates to this backend?</label
            >
            <div class="control">
              <input
                id="backend_export"
          ...
          <div class="field">
            ...
            <div class="control">
              <input
                ...
                v-model="backup_data"
              />
              <label for="backup_data" class="is-unselectable">{{
                backup_data ? 'Yes' : 'No'
              }}</label>
            </div>
            <p class="help is-bold">
              This will run a one time backup for the backend data. (Will run before the forced
          ...
          <div class="field" v-if="backends.length < 1">
            ...
            <div class="control">
              <input
                ...
                type="checkbox"
                class="switch is-success"
                v-model="force_import"
              />
              <label for="force_import" class="is-unselectable">{{
                force_import ? 'Yes' : 'No'
              }}</label>
            </div>
            <p class="help">
          ...
          <div class="field" v-if="backends.length > 0">
            ...
            <div class="control">
              <input
                ...
                type="checkbox"
                class="switch is-success"
                v-model="force_export"
              />
              <label for="force_export" class="is-unselectable">{{
                force_export ? 'Yes' : 'No'
              }}</label>
            </div>
            <p class="help has-text-danger is-bold">
...
const emit =
  defineEmits<
    (e: 'addBackend' | 'backupData' | 'forceExport' | 'forceImport', backend: Backend) => void
  >();
...
const hasPlexOauth = computed<boolean>(
  () => !!plex_oauth.value && 0 < Object.keys(plex_oauth.value).length,
);
...
const getUsers = async (
  ...
  try {
    ...
    const optionKeys: Array<keyof BackendOptions> = [
      'ADMIN_TOKEN',
      'plex_guest_user',
      'PLEX_USER_PIN',
      'is_limited_token',
