    ...
    <div>
        <div class="columns is-multiline">
                                    ...
                                    <div class="column is-4-tablet is-6-mobile has-text-right-mobile">
                                        <div class="is-text-overflow">
                                            ...
                                            <span v-if="history?.metadata && Object.keys(history?.metadata).length > 1"
                                                ...
                                                (<span class="has-tooltip">+{{ Object.keys(history.metadata).length - 1
                                                    }}</span>)
                                            </span>
                                        </div>
                                    </div>
            ...
            <div class="column is-12" v-for="log in logs" :key="log.filename">
                ...
                <code class="box logs-container"><span class="is-block" v-for="(item, index) in log.lines" :key="log.filename + '-' + index">
                        <template v-if="item?.date">[<span class="has-tooltip" v-tooltip="`${moment(item.date).format(TOOLTIP_DATE_FORMAT)}`">
                                {{ moment(item.date).format('HH:mm:ss') }}
                            </span>]
                        </template><span>{{ item.text }}</span></span></code>
            </div>
...
const transformDate = str => {
    ...
    const mm = moment(isoDate)

    return str.replace(dateRegex, `[<span class="has-tooltip" v-tooltip="${mm.format(TOOLTIP_DATE_FORMAT)}">${mm.format('HH:mm:ss')}</span>]`)
}
</script>
