...
| WS_CRON_IMPORT           | bool   | Enable import task. Value casted to bool.                                     | `false`                       |
| WS_CRON_IMPORT_AT        | string | When to run import task. Valid Cron Expression Expected.                      | `0 */1 * * *` (Every 1h)      |
| WS_CRON_IMPORT_ARGS      | string | Flags to pass to the import command.                                          | `-v`                          |
| WS_CRON_EXPORT           | bool   | Enable export task. Value casted to bool.                                     | `false`                       |
| WS_CRON_EXPORT_AT        | string | When to run export task. Valid Cron Expression Expected.                      | `30 */1 * * *` (Every 1h 30m) |
| WS_CRON_EXPORT_ARGS      | string | Flags to pass to the export command.                                          | `-v`                          |
| WS_CRON_PUSH             | bool   | Enable push task. Value casted to bool.                                       | `false`                       |
| WS_CRON_PUSH_AT          | string | When to run push task. Valid Cron Expression Expected.                        | `*/10 * * * *` (Every 10m)    |
| WS_CRON_PUSH_ARGS        | string | Flags to pass to the push command.                                            | `-v`                          |
| WS_CRON_PRUNE            | string | Enable prune task. Value is casted to bool.                                   | `true`                        |
| WS_CRON_PRUNE_AT         | string | When to run prune task. Valid Cron Expression Expected.                       | `0 */12 * * *` (Every 12h)    |
