...
if (!function_exists('compute_array_diff')) {
    /**
     * Compute the difference from new data to old data.
     * The logic flow is if data has changed, or added in $new_data,
     * it will be added to the $difference array. data that exists in
     * $old_data but not in $new_data will be ignored. from the computation.
     * However, There is a downside to it, it does not handle data deletion.
     *
     * @param array $old_data The old array.
     * @param array $new_data The new array.
     *
     * @return array Returns the keys which has changed or added by the new data.
     */
    function compute_array_diff(array $old_data, array $new_data): array
    {
        $difference = [];

        foreach ($new_data as $key => $value) {
            if (is_array($value)) {
                if (!isset($old_data[$key]) || !is_array($old_data[$key])) {
                    $difference[$key] = $value;
                } else {
                    $new_diff = compute_array_diff($old_data[$key], $value);
                    if (!empty($new_diff)) {
